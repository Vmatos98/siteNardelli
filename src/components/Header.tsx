        ...
        <nav className="hidden md:flex items-center gap-2 lg:gap-4 font-medium text-xs lg:text-sm text-slate-600">
          <div className="flex items-center gap-3 lg:gap-4">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.path}
                className={`transition-colors relative group py-1 whitespace-nowrap ${isActive(link.path) && link.path !== '/#contato'
          ...
          <div className="flex items-center gap-2 lg:gap-3 ml-2 shrink-0">
            <Link
              ...
              className={`px-3 lg:px-4 py-1.5 lg:py-2 border border-orange-600 text-orange-600 rounded hover:bg-orange-50 transition-colors whitespace-nowrap font-semibold ${isActive('/trabalhe-conosco') ? 'bg-orange-50 ring-2 ring-orange-200' : ''
                }`}
            >
              Trabalhe Conosco
            </Link>
          ...
          <div className="flex items-center gap-2 lg:gap-3 border-l pl-3 lg:pl-4 border-slate-200 h-8 shrink-0">
            <a href="https://www.instagram.com/nardelliusinagem?igsh=MWFia2dqemxpN2NxNA==" target="_blank" rel="noopener noreferrer" className="text-pink-600 hover:text-pink-700 transition-colors transform hover:scale-110" title="Instagram">
        ...
        <div className="py-2">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.path}
              className={`block px-6 py-3 hover:bg-orange-50 hover:text-orange-600 transition-colors ${isActive(link.path) && link.path !== '/#contato' ? 'text-orange-600 font-bold bg-orange-50' : 'text-slate-700'
                }`}
          ...
          <Link
            href="/trabalhe-conosco"
            onClick={() => setIsMobileMenuOpen(false)}
            className={`block mx-4 my-2 px-4 py-3 border border-orange-600 text-orange-600 font-semibold rounded-lg text-center hover:bg-orange-50 transition-all ${isActive('/trabalhe-conosco') ? 'bg-orange-50 ring-2 ring-orange-200' : ''
              }`}
          >
            Trabalhe Conosco
          </Link>
